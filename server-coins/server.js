...
app.get('/commemorative_coins', (req, res) => {
    connection.query('SELECT * FROM catalog.coins;',
        (err, data) => {
...
app.get('/coins/:id', (req, res) => {
    connection.query(`SELECT * FROM catalog.coins WHERE id = ${req.params.id};`,
        (err, data) => {
            if (err) return res.status(500);
            res.json(data);
        })
...
app.get('/coin/:id', (req, res) => {
    connection.query(`SELECT * FROM catalog.coins WHERE Title REGEXP '^${req.params.id}'
